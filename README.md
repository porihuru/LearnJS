...
2. `F5` を押します
3. 「問題集をデバッグ」を選ぶと、ローカルサーバーとEdgeが起動します

### コマンドから起動する場合

1. ターミナルで `node server.js` を実行します
2. ブラウザで `http://127.0.0.1:5500/index.html` を開きます

起動後、次の手順で利用します。

1. 初回起動時にCSVを読み込み、カテゴリが作成されます
2. カテゴリを選び、以下のいずれかで開始します
   - **ランダムスタート**：`N問` を指定して開始
   - **IDからスタート**：開始IDと`N問`を指定して開始

※CSVをXHRで読み込むため、`index.html` を `file://` で直接開くとChrome/Edgeのセキュリティ制限により読み込めません。必ず上記の方法で `http://` 経由で開いてください。
