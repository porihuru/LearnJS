...
function buildItem(row) {
  ...
  // [IDX-020-01] trim（Util.trim が無い場合は String(...).replace(...) でも可）
  var c1 = Util.trim(row.choice1 || "");
  var c2 = Util.trim(row.choice2 || "");
  var c3 = Util.trim(row.choice3 || "");
  var c4 = Util.trim(row.choice4 || "");
